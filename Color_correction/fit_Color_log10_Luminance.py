...
for sizes in sizes_list:
    ...
    Luminance_list = np.log10(Luminance_list)
...
with open(f'fit_L_C_parameters/Color_log10_Luminance_Fit_result_poly_{fit_degree}.json', 'w') as fp:
    json.dump(json_save, fp)
L_gamma_2_2 = max_luminance * (pixel_value_list) ** 2.2
L_gamma_2_4 = max_luminance * (pixel_value_list) ** 2.4
L_gamma_2_6 = max_luminance * (pixel_value_list) ** 2.6
plt.plot(pixel_value_list, np.log10(L_gamma_2_2), '--', label=f'Gamma = 2.2')
plt.plot(pixel_value_list, np.log10(L_gamma_2_4), '--', label=f'Gamma = 2.4')
plt.plot(pixel_value_list, np.log10(L_gamma_2_6), '--', label=f'Gamma = 2.6')
