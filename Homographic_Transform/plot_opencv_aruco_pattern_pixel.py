...
# num_rows = int((display_height_pixel + spacing) / (marker_size + spacing))  # 行数
# num_cols = int((display_width_pixel + spacing) / (marker_size + spacing))  # 列数
...
# 计算网格总宽高
grid_width = num_cols * marker_size + (num_cols - 1) * spacing
grid_height = num_rows * marker_size + (num_rows - 1) * spacing
...
for row in range(num_rows):
    for col in range(num_cols):
        ...
        # 计算标记放置位置
        x_offset = x_start + col * (marker_size + spacing)
        y_offset = y_start + row * (marker_size + spacing)
