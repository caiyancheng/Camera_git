...
root_path = r'E:\sony_pictures\Homo_Aruco_2025_2_19_whole_process'
...
for json_file in tqdm(json_file_list):
    ...
    exr_data = exr.get()
    image = (exr_data / 1000.0 * 255).clip(0, 255).astype(np.uint8)
    h, w = image.shape[:2]
    # white_mask = np.ones([h, w], dtype=np.uint8)
    white_mask = np.ones([h, w, 3], dtype=np.float32)
    ...
    # mask = aligned_white_mask == 0
    # aligned_image_compute = np.ma.masked_array(aligned_image, mask=np.repeat(mask[:, :, np.newaxis], 3, axis=2))
...
# aligned_image_stack_array = np.ma.stack(aligned_image_compute_list, axis=0)
# aligned_image_average_array = np.ma.mean(aligned_image_stack_array, axis=0, dtype=np.float16).filled(0)
# 计算所有符合条件的索引
