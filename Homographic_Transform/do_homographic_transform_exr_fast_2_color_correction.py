...
json_file_name = 'homography_aruco_result_aruco_exr.json'
# image_list = ['color_fringing_letter.exr']
image_list = ['color_fringing_black_circle.exr', 'aruco.exr', 'color_fringing_white_circle.exr']
...
obj_points[:, 0] = obj_points[:,
                   0] + display_width_pixel / 2  # - 1/2 #将起始位置由中点放到左上角，并反转y轴坐标,注意由于Homographic Transform时候是以像素间为中心的，所以这里要少1/2
obj_points[:, 1] = -obj_points[:, 1]
obj_points[:, 1] = obj_points[:,
                   1] + display_height_pixel / 2  # - 1/2 #obj_pints位于两个像素之间（由homographic transform的定义可以知道）
...
for img_name in tqdm(image_list):
    ...
    cv2.imwrite(os.path.join(root_path, f'CC_sRGB_Homo_Transform_{img_pure_name}_center_crop.png'), cropped_image_rgb_uint8[:, :, ::-1])
    out_path = os.path.join(root_path, f'CC_XYZ_Homo_Transform_{img_pure_name}_center_crop.exr')
    np.savez_compressed(os.path.join(root_path, f"CC_XYZ_Homo_Transform_{img_pure_name}_center_crop.npz"), cropped_image=cropped_image_xyz_linear)
