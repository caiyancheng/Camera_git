...
def register_checkerboard(points):
    ...
    输出：
      ...
      block_centers: (250,2) 的 numpy 数组，每个簇（白块）的中心坐标（用于后续可视化）
    """
    # ----- 第一步：将 16000 个白子像素聚成 250 个白块簇  -----
    n_blocks = 250
    kmeans_blocks = KMeans(n_clusters=n_blocks, random_state=0).fit(points)
    block_labels = kmeans_blocks.labels_          # 每个点的簇号（0～249）
    ...
    proj = centers_centered.dot(Vt.T)  # 转换到新坐标系下, shape=(250,2)
    ...
    return registration, block_centers
...
if __name__ == '__main__':
    ...
    coordinates = fast_peak_local_max(gray, min_distance=3, threshold_abs=220)  # 根据实际情况调整阈值
    points = coordinates[:, [1, 0]].astype(float)  # 转换为 (x, y)
    if points.shape[0] != 25*20*8*8/2:
        raise ValueError("检测的点数不对，请重新调整参数！")

    # 3. 执行配准，返回 registration 和 block_centers
    reg, block_centers = register_checkerboard(points)
    print("部分配准结果：")
    print(reg[:10])
    ...
    image_vis[expanded_mask > 0] = [0, 0, 255]  # 标记为红色
    ...
    print(f"局部极大值结果已保存至：{output_path}")

    # 5. 可视化 block_centers（聚类得到的白块中心，用绿色圆点标记）
    image_centers_vis = image.copy()
    for center in block_centers:
        # block_centers 是 (x,y)，转换为整数坐标
        center_int = (int(round(center[0])), int(round(center[1])))
        cv2.circle(image_centers_vis, center_int, radius=5, color=(0, 255, 0), thickness=-1)
    output_centers_path = os.path.join(dir_name, f"{name}_block_centers{ext}")
    cv2.imwrite(output_centers_path, image_centers_vis)
    print(f"block_centers 可视化结果已保存至：{output_centers_path}")
